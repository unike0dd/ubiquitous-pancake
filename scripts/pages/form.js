export function renderForm(page, text, services) {
  ...
  const fields = (page.fields || [])
    .map(([label, help]) => {
      ...
      const isLong =
        safeLabel.includes("Message") ||
        safeLabel.includes("Summary") ||
        safeLabel.includes("Experience");
      const tag = isLong
        ? `<textarea aria-label="${text(safeLabel)}"></textarea>`
        : `<input type="text" aria-label="${text(safeLabel)}" />`;
      return `<div class="field ${isLong ? "full" : ""}"><label>${text(safeLabel)}</label>${tag}<span class="help">${text(help || "")}</span></div>`;
    })
    .join("");
