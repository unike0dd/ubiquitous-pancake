export function renderHome(page, text) {
  const services = page.services.map((service, index) => `
  ...
  const metrics = page.metrics.map(([label, copy]) => `<div class="metric"><strong>${text(label)}</strong><span>${text(copy)}</span></div>`).join("");
