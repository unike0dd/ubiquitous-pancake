function getLearningKey(title) {
  ...
  if (lower.includes("logistics") || lower.includes("logística")) return "logistics";
...
export function renderService(page, text) {
  const covers = page.covers.map((item, index) => `<article class="sample-card"><div class="number">${String(index + 1).padStart(2, "0")}</div><h3>${text(item)}</h3><p>Structured support applied to recurring operational execution.</p></article>`).join("");
  const outcomes = page.outcomes.map((item) => `<span class="pill">${text(item)}</span>`).join("");
